...
```
mesh_discover → 2 agents online (cat-cafe-node + mesh-echo-node)
...
- [ ] echo-node 只是回声，需要接入真实 agent 能力
- [ ] heartbeat 逻辑在 sidecar/echo-node 里手写，应该下沉到 MeshClient
