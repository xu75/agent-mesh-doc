...
ANTHROPIC_BASE_URL=http://nas:3020/v1 claude code

# Cursor 使用 pool 的 GPT bridge
# 在 Cursor 设置中将 OpenAI endpoint 指向 http://nas:3020/v1
```
...
CREATE TABLE teams (
  ...
  default_borrow_limit_usd REAL NOT NULL DEFAULT 3.0, -- 新成员默认透支上限（USD）
...
**冷启动示例**（`default_borrow_limit_usd = 3.0`）：

```
新成员小明加入，balance = $0，borrow_limit = $3
可用额度 = $0 + $3 = $3（可以先用）

小明消耗了 $1 → balance = -$1，可用 = -$1 + $3 = $2（继续用）
小明的 bridge 为别人服务了 $2 → balance = -$1 + $2 = $1，可用 = $1 + $3 = $4
小明又消耗了 $4 → balance = $1 - $4 = -$3，可用 = -$3 + $3 = $0（触顶，需要贡献了）
管理员给小明加授信 → borrow_limit_override = $5 → 可用 = -$3 + $5 = $2（又能用了）
```
