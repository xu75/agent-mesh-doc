...
// bridge register payload
{
  ...
  // Owner 自路由（AC-31 修订，2026-04-15 CVO 决策）
  allowOwnerRoute?: boolean,           // 默认 false；cli-chat 强制忽略
  selfRouteMaxConcurrency?: number,    // 默认 1（仅 allowOwnerRoute=true 时生效）
  capabilities: {
...
```
maxConcurrency = externalMaxConcurrency + selfRouteMaxConcurrency（自动计算）
总活跃 = externalActiveSessions + selfRouteActiveSessions ≤ maxConcurrency
```

Owner 自路由不走 shareMode/external 检查（owner 用自己的 bridge 不受 idle-only 约束）。

**自路由计费规则**（2026-04-15 CVO 决策）：
- 自路由 = 可观测但零金额流量
- `normalized_cost=0`, `cost_source='self_route'`, `is_self_route=true`
- 预留（pending）、结算（settled）、超时（expired）金额均为 $0
- 不计入 quota（`member_balances`），不计入 rate-limit（5H/7D）
- 审计记录保留（token 用量、model 等可查询）
- 防滥用：受 `selfRouteMaxConcurrency` 并发限制
...
- 自路由条件放行（AC-31 修订）：`oauth-proxy` bridge 可声明 `allowOwnerRoute=true`，允许 owner 路由到自己的 bridge（多机器漫游场景）。`cli-chat` bridge 始终禁止自路由。自路由请求零金额记账，不影响 quota/rate-limit
...
- [x] AC-P2-11: 自路由条件放行——`oauth-proxy` bridge 可声明 `allowOwnerRoute=true` 启用 owner 自路由；`cli-chat` 始终禁止。自路由独立并发计数 + 零金额记账
